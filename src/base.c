                             ...
                             const unsigned int *v)
{
    ...
    if (graph->directed) {
        ...
        while (gnx_set_iter_next(&iter, &wptr)) {
            w = *((unsigned int *)wptr);
                           ...
                           const unsigned int *v)
{
    ...
    if (graph->directed) {
        ...
        while (gnx_set_iter_next(&iters, &wptr)) {
            w = *((unsigned int *)wptr);
            noded_w = (GnxNodeDirected *)(graph->graph[w]);
            g_assert(noded_w);
                         ...
                         unsigned int *w)
{
    ...
    if (iter->weighted)
        ...
        success = gnx_set_iter_next(&(iter->set), &wptr);
                           ...
                           unsigned int *w)
{
    ...
    *has_head = FALSE;
    if (iter->weighted) {
        ...
        while (gnx_set_iter_next(&(iter->set), &wptr)) {
            *w = *((unsigned int *)wptr);
            if (iter->i <= *w) {
                *has_head = TRUE;
                break;
                ...
                const unsigned int *v)
{
    ...
    g_return_val_if_fail(graph->weighted, GNX_FAILURE);
             ...
             const GnxBool weighted)
{
    ...
    graph->weighted = GNX_WEIGHTED & weighted;
